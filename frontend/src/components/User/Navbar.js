...
const Navbar = ({details}) => {
  ...
  return (
    ...
    <Link style={{textDecoration:'none',color:'white'}} to='/'><h3 style={{marginLeft:'1rem'}}>Welcome {details.name}</h3></Link> 
      <Tooltip title="Account settings" sx={{mr:'1rem'}} >
        <IconButton
          onClick={handleClick}
          size="small"
          sx={{ ml: 2 }}
        ...
        >
          <Avatar sx={{ width: 32, height: 32,mr:'1rem' }} src={details.profilePic}><AccountCircleIcon/></Avatar>
        </IconButton>
      </Tooltip>
    </Box>
    <Menu
